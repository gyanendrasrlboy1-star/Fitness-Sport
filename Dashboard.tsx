...
export const Dashboard: React.FC = () => {
  ...
  const workoutPercent = Math.round((dailyStats.todayWorkoutMinutes / dailyStats.goalWorkoutMinutes) * 100);
  const stepsPercent = Math.round((dailyStats.stepsCurrent / dailyStats.stepsGoal) * 100);
  const waterPercent = Math.round((dailyStats.waterGlasses / dailyStats.waterGoalGlasses) * 100);
  const caloriesPercent = Math.round((dailyStats.caloriesBurned / dailyStats.caloriesGoal) * 100);
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gradient-to-r from-primary-600/10 via-teal-500/10 to-secondary-500/10 p-6 rounded-3xl border border-primary-500/20 backdrop-blur-md">
        <div>
          ...
          <p className="text-xs md:text-sm text-slate-600 dark:text-slate-300 mt-1 max-w-xl">
            You are on a <strong className="text-amber-600 dark:text-amber-400">{profile.streakDays}-day streak!</strong> Crush your 40-minute goal today to keep IIT Bombay at #1 on the national leaderboard.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
        {/* Today's Goal Card */}
        <Card hoverEffect className="lg:col-span-1 border-primary-500/30 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Today's Goal
              </span>
              <Badge variant="green" size="sm">{workoutPercent}%</Badge>
            </div>
            <div className="mt-4 flex items-center justify-between">
              <div>
                <h3 className="text-3xl font-black text-slate-900 dark:text-white">
                  {dailyStats.todayWorkoutMinutes} <span className="text-sm font-normal text-slate-500">/ {dailyStats.goalWorkoutMinutes} min</span>
                </h3>
        ...
        {/* Fitness Score Card */}
        <Card hoverEffect className="lg:col-span-1 border-teal-500/30 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Campus Fitness Score
              </span>
              <Badge variant="teal" size="sm">Top 5%</Badge>
            </div>
            <div className="mt-4 flex items-center justify-between">
              <div>
                <h3 className="text-3xl font-black text-slate-900 dark:text-white">
                  82 <span className="text-sm font-normal text-slate-500">/ 100</span>
                </h3>
        ...
        {/* Steps Tracker Card */}
        <Card hoverEffect className="border-blue-500/30 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Today's Steps
            ...
            <div className="mt-3">
              <h3 className="text-2xl font-black text-slate-900 dark:text-white">
                ...
                <span className="text-xs font-normal text-slate-500"> / {dailyStats.stepsGoal.toLocaleString()}</span>
              </h3>
          ...
          <div className="mt-3 text-xs text-slate-500 dark:text-slate-400 flex items-center justify-between">
            ...
            <span className="font-semibold text-blue-600 dark:text-blue-400">{stepsPercent}%</span>
          </div>
        </Card>

        {/* Water Intake Card (Interactive) */}
        <Card hoverEffect className="border-cyan-500/30 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400">
                Water Intake
            ...
            <div className="mt-3">
              <h3 className="text-2xl font-black text-slate-900 dark:text-white">
                ...
                <span className="text-xs font-normal text-slate-500"> / {dailyStats.waterGoalGlasses} Glasses</span>
              </h3>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card rounded-2xl p-4 border border-slate-200 dark:border-slate-800 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-slate-900 dark:text-white">
              {dailyStats.caloriesBurned} <span className="text-xs font-normal text-slate-400">kcal</span>
            </div>
            <span className="text-[10px] text-emerald-600 font-medium">Target: {dailyStats.caloriesGoal} kcal</span>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-4 border border-slate-200 dark:border-slate-800 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-slate-900 dark:text-white">
              {profile.bmi} <span className="text-xs font-normal text-emerald-500">Normal</span>
            </div>
        ...
        <div className="glass-card rounded-2xl p-4 border border-slate-200 dark:border-slate-800 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-slate-900 dark:text-white">
              {dailyStats.restingHeartRate} <span className="text-xs font-normal text-slate-400">BPM</span>
            </div>
            <span className="text-[10px] text-emerald-600 font-medium">Athletic Range</span>
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-4 border border-slate-200 dark:border-slate-800 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-lg font-bold text-slate-900 dark:text-white">
              {dailyStats.sleepHours} <span className="text-xs font-normal text-slate-400">/ 8 hrs</span>
            </div>
            ...
            {activeChartTab === 'weekly' ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={weeklyActivityData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="colorMinutes" x1="0%" y1="0%" x2="0%" y2="100%">
        ...
        <div className="space-y-4">
          <Card className="border-secondary-500/30">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-bold">Emergency SOS & Campus Medical</span>
              </div>
          ...
          <Card className="bg-gradient-to-br from-slate-900 to-slate-800 text-white border-0">
            ...
            <p className="text-xs text-slate-300 mt-1 line-clamp-2">
              Complete 25 mins of AI workout 24 days out of 30 to win hostel cafeteria vouchers.
            </p>
            <div className="w-full bg-slate-700 h-2 rounded-full overflow-hidden mt-3">
              <div className="bg-primary-400 h-full rounded-full" style={{ width: '65%' }} />
            </div>
                ...
                {workoutList.slice(0, 4).map((session) => (
                  <tr key={session.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800/40 transition-colors">
                    <td className="py-3 font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                      ...
                      <span className="truncate max-w-[140px] sm:max-w-none">{session.exerciseName}</span>
                    </td>
            ...
            {upcomingCampusEvents.map((evt) => (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <Badge variant="blue" size="sm">{evt.badge}</Badge>
                </div>
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  {ach.unlocked && <span className="text-[10px] text-amber-500 font-semibold">+{ach.xpReward} XP</span>}
                </div>
