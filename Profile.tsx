...
export const Profile: React.FC = () => {
  ...
  const [campusLeaderboardVisible, setCampusLeaderboardVisible] = useState(true);
  ...
  const calculatedBmi = Number((formData.weightKg / Math.pow(formData.heightCm / 100, 2)).toFixed(1));
    ...
    <div className="space-y-8 pb-12">
      ...
      <div>
        ...
        <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400">
          Manage your collegiate credentials, biometric profile, and privacy preferences.
        </p>
      </div>
          ...
          <div className="flex items-end gap-4">
            ...
            <div className="mb-1">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="green" size="sm">Level {profile.level}</Badge>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <form onSubmit={handleSave} className="space-y-4 text-xs">
            ...
            <div className="pt-2">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {['Muscle Gain', 'Fat Loss', 'Stamina & Energy', 'Stress Relief & Posture'].map((goal) => {
                  const isSelected = formData.fitnessGoal === goal;
                  return (
                    <button
                      key={goal}
                      type="button"
                      onClick={() => setFormData({ ...formData, fitnessGoal: goal as any })}
        ...
        <div className="space-y-6">
          <Card className="space-y-4">
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
              <div className="flex items-center gap-2.5">
                {isDarkMode ? <Moon className="w-4 h-4 text-amber-400" /> : <Sun className="w-4 h-4 text-slate-500" />}
                <div>
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-800">
              ...
              <button
                type="button"
                onClick={() => setCampusLeaderboardVisible(!campusLeaderboardVisible)}
                className={`w-11 h-6 flex items-center rounded-full p-1 transition-colors ${
          ...
          <Card className="border-teal-500/30 bg-teal-50/10 text-center p-5 space-y-2">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Verified Student Innovator in Problem Statement <strong>SIH26196</strong>.
            </p>
          </Card>
        </div>
      </div>
